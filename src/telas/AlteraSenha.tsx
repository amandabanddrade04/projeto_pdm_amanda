  ...
  .shape({
    senha_atual: yup
      ...
      .matches(
        ...
        'Senha deve conter ao menos 8 caracteres, uma letra maiúscula, uma letra minúscula, um número e um caractere especial',
      ),
    nova_senha: yup
      .string()
      .required(requiredMessage)
    ...
    confirmar_senha: yup
      .string()
      .required(requiredMessage)
      .equals([yup.ref('nova_senha')], 'As senhas devem ser iguais'),
  })
  .required();
...
export default function AlteraSenha({navigation}: any) {
  ...
  const {userAuth, signIn, alteraSenha} = useContext<any>(AuthContext);
  ...
  async function onSubmit(data: {
    ...
    if (data.nova_senha !== data.confirmar_senha) {
      const msge = await alteraSenha(data.nova_senha);
      if (msge === 'ok') {
        setRequisitando(false);
      ...
      setRequisitando(false);
      setDialogVisivel(true);
      setMensagem({tipo: 'erro', mensagem: 'As senhas não conferem'});
    }
  }
          ...
          <TextInput
            ...
            right={
              <TextInput.Icon
                icon="eye"
                color={exibirSenha ? theme.colors.onBackground : theme.colors.error}
                onPress={() => setExibirSenha(previous => !previous)}
              />
            }
          />
        )}
